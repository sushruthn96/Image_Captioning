...
class Experiment(object):
    ...
    def __init__(self, net, train_set, val_set, optimizer, stats_manager,
        ...
        train_loader = td.DataLoader(train_set, batch_size=batch_size, shuffle=True,
                                     drop_last=True, pin_memory=True, collate_fn = data_loader.coco_batch)
    ...
    def run(self, encoder_net, num_epochs, save_step=1, log_step=1, plot=None):
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for tmp, (images, captions, lengths) in enumerate(self.train_loader):
                images, captions = images.to(self.net.device), captions.to(self.net.device)
                features = encoder_net.forward(images)
                targets = pack_padded_sequence(captions, lengths, batch_first=True)[0]
                
                self.optimizer.zero_grad()
                y = self.net.forward(features, captions, lengths)
                loss = self.net.criterion(y, targets)
                loss.backward()
                self.optimizer.step()
                with torch.no_grad():
                    self.stats_manager.accumulate(loss.item(), features, y, targets)
                
                if (tmp+1) % log_step == 0:
                    print('Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}'.format(epoch, num_epochs, tmp+1, total_step, loss.item())) 

                    try:
                        f_abc = open(os.path.join(self.output_dir, "log.txt"),"a")
                        f_abc.write("Epoch: " + str(epoch) + "/" + str(num_epochs)+ " Step: "+ str(tmp+1)+ "/"+ str(total_step)+ ", loss: "+str(loss.item())+ "\n")
                        f_abc.close()
                    except Exception as e:
                        print(e)     
                if (tmp+1) % save_step == 0:
                    torch.save(self.net.state_dict(), os.path.join(self.output_dir, 'decoder-{}-{}.ckpt'.format(epoch+1, tmp+1)))
